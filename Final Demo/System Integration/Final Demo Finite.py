...
FORWARD_STEP_DISTANCE = 7       # The distance the robot should drive forward after each turn
crossFound = False              # Flag to denote whether the cross at the end of the path was found
VIEW_SHIFT_ANGLE = 5            # The angle the robot should turn in the update angle state if the tape is not found
ANGLE_FUDGE_DELTA = 0           # The minimum angle the robot should attempt to turn to in the update angle state
ANGLE_AFTER_START = -22         # The angle the robot should rotate to after moving to the start
...
def measure_angle():
    ...
    while (consecutiveSameAngleCount < MIN_CONSECUTIVE_SAME_ANGLE_COUNT):
        mask = take_picture(4.0,1.0)#Crop values for the picture
...
def measure_start_angle():
    ...
    # Wait until tape angle reported is the same for MIN_CONSECUTIVE_SAME_ANGLE_COUNT consecutive loops before returning angle
    while (consecutiveSameAngleCount < MIN_CONSECUTIVE_SAME_ANGLE_COUNT):
        mask = take_picture(2.0,1.0)#Crop values for the picture
#        cv.imshow('frame',mask)
        #--------------------------------------\/Image Measurment\/--------------------------------------
        nonzero = np.nonzero(mask)
        if len(nonzero[0])==0:
...
def state_drive_to_start():
    ...
    distToStart = 17  # The sqrt of two times 12 inches    
...
#    nextAngle = function that finds next angle
    ...
    elif nextAngle > ANGLE_FUDGE_DELTA:
...
def state_check_for_cross:
    ...
    crossFound = False    # NOTE: Placeholder. Implement function to set this flag
    return state_drive_forward
